...
export async function findMenusByDate(date) {
    const [rows] = await pool.query(`SELECT c.id, c.escola_id AS schoolId, c.dia_semana AS dayOfWeek,
      ...
      COALESCE(GROUP_CONCAT(ci.nome ORDER BY ci.id SEPARATOR '||'), c.ingredientes, '') AS ingredients
         ...
         WHERE c.dia_semana = WEEKDAY(:date) + 1 AND c.ativo = 1
         GROUP BY c.id, c.escola_id, c.dia_semana, e.nome, c.etapa_ensino, c.turno,
...
export async function createMenu(input) {
    ...
    try {
        ...
        for (const schoolId of input.schoolIds) {
          ...
          (escola_id, nutricionista_id, dia_semana, etapa_ensino, turno, refeicao, nome_prato, ingredientes, ativo)
         VALUES (:schoolId, :nutritionistId, :dayOfWeek, :educationStage, :shift, :meal, :dish, :ingredients, :active)`, {
                schoolId,
