...
export async function submitEvaluation(menuId: number, ratings: Ratings, suggestion: string, classId: number, educationStage: string, ingredients: string[]) {
  const response = await fetch(`${API_URL}/api/student/evaluations`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ menuId, classId, educationStage, ingredients, ...ratings, suggestion: suggestion || undefined }),
  });
